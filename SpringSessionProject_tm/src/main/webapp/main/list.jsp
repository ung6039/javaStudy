...
<body>
	...
	/*
		<App name = "홍길동"> => new App("홍길동"); ==> 변경할 수 없음
		props : 값을 넘기거나 저장하는 것 (property)
		state : 변경이 가능한 값 (변수)
		...
		=> <App/>
			...
			완료가 된 다음 => 화면을 변경 ==> render()
					 	========
						rendering => setSatate()  :: render를 다시 호출
	function H(){
	...
	const H=()=>{
		...
		*/
		class App extends React.Component{
			...
			{
				// 서버 연결
				axios.get('http://localhost:808/web/main/music.do').then((result)=>{
			...
			render(){
				return (
					<div className="row">
						...
						<MusicTable moive={this.state.music} ss={this.state.ss}/>
					</div>
				)
			}
		}
